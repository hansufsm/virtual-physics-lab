...
const SUGGESTIONS = [
  "Qual a diferença entre meia-vida e vida média?",
  "Como datar uma amostra com C-14 se restam 25% dos núcleos?",
  "Por que a atividade A é proporcional a N?",
  "Por que o decaimento é exponencial?",
];

export const DecayAssistantTab = ({ params, results }: Props) => {
  ...
  const send = async (text: string) => {
    ...
    try {
      ...
      const summary = {
        isotopo: params.isotopeName,
        T_half_s: params.halfLifeS,
        lambda_per_s: results.lambda,
        meanLife_s: results.meanLifeS,
        N0: params.N0,
        t_s: params.timeS,
        N_t: results.N,
        atividade_Bq: results.activity,
        fracao_restante: results.fractionRemaining,
      };
      const resp = await fetch(url, {
        ...
        body: JSON.stringify({
          ...
          context: { experimento: "Decaimento radioativo", parametros: params, resultados: summary },
        }),
      });
    ...
    <div className="flex flex-col h-[600px] rounded-xl border border-border bg-card shadow-card overflow-hidden">
      <div className="flex items-center gap-2 px-5 py-3 border-b border-border bg-gradient-subtle">
        ...
        <div>
          ...
          <p className="text-xs text-muted-foreground">Conhece o isótopo, λ, N(t), A(t) e o cenário atual</p>
        </div>
      </div>
      <div ref={scrollRef} className="flex-1 overflow-y-auto p-4 space-y-3">
        {messages.length === 0 && (
          <div className="space-y-3 animate-fade-in">
            <p className="text-sm text-muted-foreground">Pergunte sobre meia-vida, atividade, datação ou estatística do decaimento.</p>
      ...
      <form className="flex items-center gap-2 p-3 border-t border-border bg-background"
        ...
        <Input value={input} onChange={(e) => setInput(e.target.value)}
          placeholder="Pergunte algo sobre decaimento radioativo..." disabled={loading} className="flex-1" />
