...
import type { TransformerParams, TransformerResults } from "@/lib/physics";

interface Msg { role: "user" | "assistant"; content: string }

interface Props { params: TransformerParams; results: TransformerResults }

const SUGGESTIONS = [
  "Por que linhas de transmissão usam alta tensão e corrente baixa?",
  "Como o fator de acoplamento k afeta V₂?",
  "O que muda se eu dobrar N₂ mantendo R_L constante?",
  "Por que a potência se conserva no transformador ideal?",
];

export const TransformerAssistantTab = ({ params, results }: Props) => {
      ...
      const resp = await fetch(url, {
        ...
        body: JSON.stringify({
          ...
          context: {
            ...
            parametros: params,
            resultados: {
              razao_a: results.ratio,
              tipo: results.type,
              V2_ideal_V: results.vSecondaryIdeal,
              V2_real_V: results.vSecondary,
              I1_A: results.iPrimary,
              I2_A: results.iSecondary,
              P_in_W: results.pIn,
              P_out_W: results.pOut,
              perdas_W: results.pLoss,
    ...
    <div className="flex flex-col h-[600px] rounded-xl border border-border bg-card shadow-card overflow-hidden">
      <div className="flex items-center gap-2 px-5 py-3 border-b border-border bg-gradient-subtle">
        ...
        <div>
          ...
          <p className="text-xs text-muted-foreground">Conhece a configuração do transformador</p>
        </div>
      </div>
        ...
        {messages.length === 0 && (
          <div className="space-y-3 animate-fade-in">
            <p className="text-sm text-muted-foreground">
              Pergunte sobre razão de transformação, perdas, acoplamento ou aplicações práticas.
            </p>
      ...
      >
        <Input
          ...
          placeholder="Pergunte algo sobre o transformador..."
          disabled={loading}
          className="flex-1"
        />
