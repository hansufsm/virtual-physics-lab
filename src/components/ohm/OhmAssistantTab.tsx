...
const SUGGESTIONS = [
  "Por que a corrente cai quando aumento o comprimento do fio?",
  "Como o ajuste linear V×I me dá a resistência?",
  "O que muda se eu trocar o cobre por níquel-cromo?",
  "Sugira uma varredura interessante para estimar ρ.",
];

export const OhmAssistantTab = ({ params, results }: Props) => {
    ...
    try {
      ...
      const resp = await fetch(url, {
        ...
        body: JSON.stringify({
          ...
          context: {
            experimento: "Lei de Ohm e resistividade",
            parametros: params,
            resultados: {
              area_m2: results.area,
              resistencia_ohm: results.resistance,
              corrente_A: results.current,
              tensao_no_resistor_V: results.voltageDrop,
              potencia_W: results.power,
              condutancia_S: results.conductance,
            },
          },
        }),
      });
    ...
    <div className="flex flex-col h-[600px] rounded-xl border border-border bg-card shadow-card overflow-hidden">
      <div className="flex items-center gap-2 px-5 py-3 border-b border-border bg-gradient-subtle">
        ...
        <div>
          ...
          <p className="text-xs text-muted-foreground">Conhece o circuito atual</p>
        </div>
      </div>
        ...
        {messages.length === 0 && (
          <div className="space-y-3 animate-fade-in">
            <p className="text-sm text-muted-foreground">
              Faça perguntas sobre Lei de Ohm, resistividade ou peça interpretação dos gráficos.
            </p>
      ...
      >
        <Input
          ...
          placeholder="Pergunte algo sobre o circuito..."
          disabled={loading}
          className="flex-1"
        />
