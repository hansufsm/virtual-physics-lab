...
import type { TransientParams, TransientResults } from "@/lib/physics";

interface Msg { role: "user" | "assistant"; content: string }
interface Props { params: TransientParams; results: TransientResults }

const SUGGESTIONS = [
  "Por que a corrente em LR não muda instantaneamente?",
  "O que significa amortecimento crítico em RLC?",
  "Como Q se relaciona com o número de oscilações visíveis?",
  "De onde vem a energia armazenada no indutor?",
];

export const TransientAssistantTab = ({ params, results }: Props) => {
  ...
  const send = async (text: string) => {
    ...
    try {
      ...
      const summary = {
        modo: params.mode, fase: params.phase, V0: params.V0,
        R: params.R, L: params.L, C: params.C,
        regime: results.regime, tau: results.tau, alpha: results.alpha,
        omega0: results.omega0, omegaD: results.omegaD,
        zeta: results.zeta, Q: results.Q,
        steadyI: results.steadyI, steadyVc: results.steadyVc,
      };
      const resp = await fetch(url, {
        ...
        body: JSON.stringify({
          ...
          context: { experimento: "Transitório LR/RLC série", parametros: params, resultados: summary },
        }),
      });
    ...
    <div className="flex flex-col h-[600px] rounded-xl border border-border bg-card shadow-card overflow-hidden">
      <div className="flex items-center gap-2 px-5 py-3 border-b border-border bg-gradient-subtle">
        ...
        <div>
          ...
          <p className="text-xs text-muted-foreground">Conhece R, L, C, ζ, Q, ω₀ e o regime atual</p>
        </div>
      </div>
      <div ref={scrollRef} className="flex-1 overflow-y-auto p-4 space-y-3">
        {messages.length === 0 && (
          <div className="space-y-3 animate-fade-in">
            <p className="text-sm text-muted-foreground">Pergunte sobre τ, regimes amortecidos, fator Q ou energia em L e C.</p>
      ...
      <form className="flex items-center gap-2 p-3 border-t border-border bg-background"
        ...
        <Input value={input} onChange={(e) => setInput(e.target.value)}
          placeholder="Pergunte algo sobre o transitório LR/RLC..." disabled={loading} className="flex-1" />
