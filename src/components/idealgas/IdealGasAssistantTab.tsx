...
const SUGGESTIONS = [
  "Por que a compressão adiabática aquece o gás mais que a isotérmica?",
  "Como deduzo PV^γ = const a partir da 1ª lei e dQ = 0?",
  "Qual o sinal do trabalho W em uma expansão isobárica?",
  "Por que ΔS = 0 em um processo adiabático reversível, mesmo com ΔT ≠ 0?",
];

export const IdealGasAssistantTab = ({ params, results }: Props) => {
  ...
  const send = async (text: string) => {
    ...
    try {
      ...
      const summary = {
        P1_Pa: results.P1, P2_Pa: results.P2,
        V2_L: results.V2, T2_K: results.T2,
        work_J: results.work, heat_J: results.heat,
        deltaU_J: results.deltaU, deltaS_JperK: results.deltaS,
        Cv: results.cv, Cp: results.cp,
      };
      const resp = await fetch(url, {
        ...
        body: JSON.stringify({
          ...
          context: { experimento: "Gás ideal (PVT)", parametros: params, resultados: summary },
        }),
      });
    ...
    <div className="flex flex-col h-[600px] rounded-xl border border-border bg-card shadow-card overflow-hidden">
      <div className="flex items-center gap-2 px-5 py-3 border-b border-border bg-gradient-subtle">
        ...
        <div>
          ...
          <p className="text-xs text-muted-foreground">Conhece n, T₁, V₁, processo e γ configurados</p>
        </div>
      </div>
      <div ref={scrollRef} className="flex-1 overflow-y-auto p-4 space-y-3">
        {messages.length === 0 && (
          <div className="space-y-3 animate-fade-in">
            <p className="text-sm text-muted-foreground">Pergunte sobre PV = nRT, trabalho, calor, entropia ou os processos quase-estáticos.</p>
      ...
      <form className="flex items-center gap-2 p-3 border-t border-border bg-background"
        ...
        <Input value={input} onChange={(e) => setInput(e.target.value)}
          placeholder="Pergunte algo sobre o gás ideal..." disabled={loading} className="flex-1" />
