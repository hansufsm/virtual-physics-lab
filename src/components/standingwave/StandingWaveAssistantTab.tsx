...
import type { StandingWaveParams, StandingWaveResults } from "@/lib/physics";

interface Msg { role: "user" | "assistant"; content: string }
interface Props { params: StandingWaveParams; results: StandingWaveResults }

const SUGGESTIONS = [
  "Por que só algumas frequências formam ondas estacionárias?",
  "Como deduzo λₙ = 2L/n a partir das condições de contorno?",
  "Por que a corda fixa-livre só tem harmônicos ímpares?",
  "Como a frequência depende de T e μ?",
];

export const StandingWaveAssistantTab = ({ params, results }: Props) => {
  ...
  const send = async (text: string) => {
    ...
    try {
      ...
      const summary = {
        ...
        wavelength_m: results.wavelength,
        period_s: results.period,
        k_rad_m: results.k,
        omega_rad_s: results.omega,
        energy_J: results.energy,
        nodes_count: results.nodes.length,
        antinodes_count: results.antinodes.length,
      };
      const resp = await fetch(url, {
        ...
        body: JSON.stringify({
          ...
          context: { experimento: "Ondas estacionárias em corda", parametros: params, resultados: summary },
        }),
      });
    ...
    <div className="flex flex-col h-[600px] rounded-xl border border-border bg-card shadow-card overflow-hidden">
      <div className="flex items-center gap-2 px-5 py-3 border-b border-border bg-gradient-subtle">
        ...
        <div>
          ...
          <p className="text-xs text-muted-foreground">Conhece L, T, μ, modo n e condições de contorno</p>
        </div>
      </div>
      <div ref={scrollRef} className="flex-1 overflow-y-auto p-4 space-y-3">
        {messages.length === 0 && (
          <div className="space-y-3 animate-fade-in">
            <p className="text-sm text-muted-foreground">Pergunte sobre modos normais, espectro ou energia.</p>
      ...
      <form className="flex items-center gap-2 p-3 border-t border-border bg-background"
        ...
        <Input value={input} onChange={(e) => setInput(e.target.value)}
          placeholder="Pergunte algo sobre a corda vibrante..." disabled={loading} className="flex-1" />
