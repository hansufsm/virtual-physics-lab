...
import type { GaussParams, GaussDerived } from "@/lib/physics";

interface Msg { role: "user" | "assistant"; content: string }
interface Props { params: GaussParams; derived: GaussDerived }

const SUGGESTIONS = [
  "Por que o fluxo total não depende do raio da gaussiana?",
  "Como aplicar simetria esférica para encontrar E(r)?",
  "Por que E é constante para um plano infinito?",
  "Como muda Q_enc dentro de uma esfera uniformemente carregada?",
];

export const GaussAssistantTab = ({ params, derived }: Props) => {
    ...
    try {
      ...
      const resp = await fetch(url, {
        ...
        body: JSON.stringify({
          ...
          context: { experimento: "Lei de Gauss / fluxo elétrico", parametros: params, derivados: derived },
        }),
      });
    ...
    <div className="flex flex-col h-[600px] rounded-xl border border-border bg-card shadow-card overflow-hidden">
      <div className="flex items-center gap-2 px-5 py-3 border-b border-border bg-gradient-subtle">
        ...
        <div>
          ...
          <p className="text-xs text-muted-foreground">Conhece a distribuição de carga e a gaussiana</p>
        </div>
      </div>
      <div ref={scrollRef} className="flex-1 overflow-y-auto p-4 space-y-3">
        {messages.length === 0 && (
          <div className="space-y-3 animate-fade-in">
            <p className="text-sm text-muted-foreground">Pergunte sobre simetria, fluxo ou escolha da superfície.</p>
      ...
      <form className="flex items-center gap-2 p-3 border-t border-border bg-background"
        ...
        <Input value={input} onChange={(e) => setInput(e.target.value)}
          placeholder="Pergunte algo sobre a lei de Gauss..." disabled={loading} className="flex-1" />
