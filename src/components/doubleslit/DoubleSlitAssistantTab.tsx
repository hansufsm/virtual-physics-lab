...
import type { DoubleSlitParams, DoubleSlitDerived } from "@/lib/physics";

interface Msg { role: "user" | "assistant"; content: string }
interface Props { params: DoubleSlitParams; derived: DoubleSlitDerived }

const SUGGESTIONS = [
  "Por que o padrão de Young é modulado por uma sinc²?",
  "Como deduzo Δy = λL/d a partir da diferença de caminho?",
  "Quantas franjas brilhantes cabem dentro do envelope central?",
  "O que muda no padrão se eu aumentar N de 2 para 10?",
];

export const DoubleSlitAssistantTab = ({ params, derived }: Props) => {
    ...
    try {
      ...
      const resp = await fetch(url, {
        ...
        body: JSON.stringify({
          ...
          context: { experimento: "Difração e interferência (fenda dupla)", parametros: params, derivados: derived },
        }),
      });
    ...
    <div className="flex flex-col h-[600px] rounded-xl border border-border bg-card shadow-card overflow-hidden">
      <div className="flex items-center gap-2 px-5 py-3 border-b border-border bg-gradient-subtle">
        ...
        <div>
          ...
          <p className="text-xs text-muted-foreground">Conhece λ, d, a, L e o ponto de prova na tela</p>
        </div>
      </div>
      <div ref={scrollRef} className="flex-1 overflow-y-auto p-4 space-y-3">
        {messages.length === 0 && (
          <div className="space-y-3 animate-fade-in">
            <p className="text-sm text-muted-foreground">Pergunte sobre franjas, envelope sinc² ou rede de difração.</p>
      ...
      <form className="flex items-center gap-2 p-3 border-t border-border bg-background"
        ...
        <Input value={input} onChange={(e) => setInput(e.target.value)}
          placeholder="Pergunte algo sobre fenda dupla / difração..." disabled={loading} className="flex-1" />
