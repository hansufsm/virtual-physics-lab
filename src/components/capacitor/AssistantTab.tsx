...
const SUGGESTIONS = [
  "Explique por que C aumenta quando insiro um dielétrico.",
  "O que muda na energia se eu dobrar a distância mantendo V fixo?",
  "Como interpretar a curva C vs d que vejo na aba de dados?",
  "Sugira uma variação de parâmetros interessante para investigar.",
];

export const AssistantTab = ({ params, results }: Props) => {
    ...
    try {
      ...
      const resp = await fetch(url, {
        ...
        body: JSON.stringify({
          ...
          context: {
            experimento: "Capacitor de placas paralelas",
            parametros: params,
            resultados: {
              capacitancia_F: results.capacitance,
              carga_C: results.charge,
              energia_J: results.energy,
              campo_eletrico_V_per_m: results.electricField,
            },
          },
        }),
      });
    ...
    <div className="flex flex-col h-[600px] rounded-xl border border-border bg-card shadow-card overflow-hidden">
      <div className="flex items-center gap-2 px-5 py-3 border-b border-border bg-gradient-subtle">
        ...
        <div>
          ...
          <p className="text-xs text-muted-foreground">Conhece o estado atual do experimento</p>
        </div>
      </div>
        ...
        {messages.length === 0 && (
          <div className="space-y-3 animate-fade-in">
            <p className="text-sm text-muted-foreground">
              Faça perguntas sobre teoria, interpretação de gráficos ou peça sugestões de experimentos.
            </p>
      ...
      >
        <Input
          ...
          placeholder="Pergunte algo sobre o capacitor..."
          disabled={loading}
          className="flex-1"
        />
