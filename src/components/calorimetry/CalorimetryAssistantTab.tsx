...
const SUGGESTIONS = [
  "Por que a temperatura fica em 0 °C enquanto o gelo derrete?",
  "Como deduzo a fórmula da temperatura de equilíbrio?",
  "Qual o papel da capacidade térmica do calorímetro?",
  "Quanto gelo precisaria para resfriar a água até 10 °C?",
];

export const CalorimetryAssistantTab = ({ params, results }: Props) => {
  ...
  const send = async (text: string) => {
    ...
    try {
      ...
      const summary = {
        ...
        meltedFraction: results.meltedFraction,
        remainingIce_kg: results.remainingIce,
        qWater_J: results.qWater,
        qSolid_J: results.qSolid,
        qIce_J: results.qIce,
        energyBalance_J: results.energyBalance,
      };
      const resp = await fetch(url, {
        ...
        body: JSON.stringify({
          ...
          context: { experimento: "Calorimetria e mudanças de fase", parametros: params, resultados: summary },
        }),
      });
    ...
    <div className="flex flex-col h-[600px] rounded-xl border border-border bg-card shadow-card overflow-hidden">
      <div className="flex items-center gap-2 px-5 py-3 border-b border-border bg-gradient-subtle">
        ...
        <div>
          ...
          <p className="text-xs text-muted-foreground">Conhece massas, calores específicos, gelo e calorímetro</p>
        </div>
      </div>
      <div ref={scrollRef} className="flex-1 overflow-y-auto p-4 space-y-3">
        {messages.length === 0 && (
          <div className="space-y-3 animate-fade-in">
            <p className="text-sm text-muted-foreground">Pergunte sobre balanço de calor, mudanças de fase ou capacidade térmica.</p>
      ...
      <form className="flex items-center gap-2 p-3 border-t border-border bg-background"
        ...
        <Input value={input} onChange={(e) => setInput(e.target.value)}
          placeholder="Pergunte algo sobre o equilíbrio térmico..." disabled={loading} className="flex-1" />
