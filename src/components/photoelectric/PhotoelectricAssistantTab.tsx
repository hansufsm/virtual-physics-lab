...
import type { PhotoelectricParams, PhotoelectricResults } from "@/lib/physics";

interface Msg { role: "user" | "assistant"; content: string }
interface Props { params: PhotoelectricParams; results: PhotoelectricResults }

const SUGGESTIONS = [
  "Por que aumentar a intensidade não aumenta K_máx?",
  "Como obter h a partir do gráfico V_s × f?",
  "Por que abaixo de f₀ não há corrente, mesmo com luz forte?",
  "Qual a diferença entre função trabalho e energia de ionização?",
];

export const PhotoelectricAssistantTab = ({ params, results }: Props) => {
  ...
  const send = async (text: string) => {
    ...
    try {
      ...
      const summary = {
        Eph_eV: results.photonEnergyEv,
        f_Hz: results.frequencyHz,
        f0_Hz: results.thresholdFreqHz,
        lambda0_nm: results.thresholdWavelengthNm,
        emits: results.emits,
        Kmax_eV: results.kMaxEv,
        Vs_V: results.stoppingVoltage,
        Isat_A: results.saturationCurrent,
        I_A: results.current,
      };
      const resp = await fetch(url, {
        ...
        body: JSON.stringify({
          ...
          context: { experimento: "Efeito fotoelétrico", parametros: params, resultados: summary },
        }),
      });
    ...
    <div className="flex flex-col h-[600px] rounded-xl border border-border bg-card shadow-card overflow-hidden">
      <div className="flex items-center gap-2 px-5 py-3 border-b border-border bg-gradient-subtle">
        ...
        <div>
          ...
          <p className="text-xs text-muted-foreground">Conhece λ, φ, V_s, K_máx e o cenário atual</p>
        </div>
      </div>
      <div ref={scrollRef} className="flex-1 overflow-y-auto p-4 space-y-3">
        {messages.length === 0 && (
          <div className="space-y-3 animate-fade-in">
            <p className="text-sm text-muted-foreground">Pergunte sobre energia de fóton, limiar de emissão ou tensão de frenagem.</p>
      ...
      <form className="flex items-center gap-2 p-3 border-t border-border bg-background"
        ...
        <Input value={input} onChange={(e) => setInput(e.target.value)}
          placeholder="Pergunte algo sobre o efeito fotoelétrico..." disabled={loading} className="flex-1" />
