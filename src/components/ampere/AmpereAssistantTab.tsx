...
import type { AmpereParams, AmpereDerived } from "@/lib/physics";

interface Msg { role: "user" | "assistant"; content: string }
interface Props { params: AmpereParams; derived: AmpereDerived }

const SUGGESTIONS = [
  "Por que B cai com 1/d em torno de um fio retilíneo?",
  "Por que correntes paralelas se atraem?",
  "Como aplico a lei de Ampère em um toroide?",
  "Por que B é praticamente zero fora do toroide?",
];

export const AmpereAssistantTab = ({ params, derived }: Props) => {
    ...
    try {
      ...
      const resp = await fetch(url, {
        ...
        body: JSON.stringify({
          ...
          context: { experimento: "Lei de Ampère (fios e toroide)", parametros: params, derivados: derived },
        }),
      });
    ...
    <div className="flex flex-col h-[600px] rounded-xl border border-border bg-card shadow-card overflow-hidden">
      <div className="flex items-center gap-2 px-5 py-3 border-b border-border bg-gradient-subtle">
        ...
        <div>
          ...
          <p className="text-xs text-muted-foreground">Conhece a geometria, correntes e ponto de prova</p>
        </div>
      </div>
      <div ref={scrollRef} className="flex-1 overflow-y-auto p-4 space-y-3">
        {messages.length === 0 && (
          <div className="space-y-3 animate-fade-in">
            <p className="text-sm text-muted-foreground">Pergunte sobre campo de fios, força entre correntes ou toroide.</p>
      ...
      <form className="flex items-center gap-2 p-3 border-t border-border bg-background"
        ...
        <Input value={input} onChange={(e) => setInput(e.target.value)}
          placeholder="Pergunte algo sobre a lei de Ampère..." disabled={loading} className="flex-1" />
