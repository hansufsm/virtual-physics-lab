...
const SUGGESTIONS = [
  "Por que o par de Helmholtz produz campo tão uniforme no centro?",
  "Como aproximar B de um solenoide finito pelo limite ideal µ₀nI?",
  "Se eu dobrar o raio mantendo I e N, o que acontece com B(0)?",
  "Como derivar Bz(z) da espira a partir da lei de Biot–Savart?",
];

export const CoilAssistantTab = ({ params, results }: Props) => {
    ...
    try {
      ...
      const resp = await fetch(url, {
        ...
        body: JSON.stringify({
          ...
          context: {
            experimento: "Campo magnético de bobinas (Biot–Savart)",
            parametros: params,
            resultados: {
              B_centro_T: results.bCenter,
              B_centro_mT: results.bCenter * 1e3,
              B_ideal_solenoide_T: results.bIdealSolenoid,
              n_espiras_por_m: results.nPerMeter,
              uniformidade_pct: results.uniformityPct,
            },
          },
        }),
      });
    ...
    <div className="flex flex-col h-[600px] rounded-xl border border-border bg-card shadow-card overflow-hidden">
      <div className="flex items-center gap-2 px-5 py-3 border-b border-border bg-gradient-subtle">
        ...
        <div>
          ...
          <p className="text-xs text-muted-foreground">Conhece a configuração da bobina</p>
        </div>
      </div>
        ...
        {messages.length === 0 && (
          <div className="space-y-3 animate-fade-in">
            <p className="text-sm text-muted-foreground">
              Pergunte sobre Biot–Savart, geometrias de bobinas, uniformidade do campo ou interpretação dos gráficos.
            </p>
      ...
      >
        <Input
          ...
          placeholder="Pergunte algo sobre o campo magnético..."
          disabled={loading}
          className="flex-1"
        />
