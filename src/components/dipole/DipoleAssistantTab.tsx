...
import type { DipoleParams, DipoleDerived } from "@/lib/physics";

interface Msg { role: "user" | "assistant"; content: string }
interface Props { params: DipoleParams; derived: DipoleDerived }

const SUGGESTIONS = [
  "Por que τ = p × E e como interpretar o sinal?",
  "Por que a força resultante é nula em campo uniforme?",
  "Como deduzir o período de pequenas oscilações?",
  "Por que o campo do dipolo decai com 1/r³?",
];

export const DipoleAssistantTab = ({ params, derived }: Props) => {
    ...
    try {
      ...
      const resp = await fetch(url, {
        ...
        body: JSON.stringify({
          ...
          context: { experimento: "Dipolo elétrico e torque", parametros: params, derivados: derived },
        }),
      });
    ...
    <div className="flex flex-col h-[600px] rounded-xl border border-border bg-card shadow-card overflow-hidden">
      <div className="flex items-center gap-2 px-5 py-3 border-b border-border bg-gradient-subtle">
        ...
        <div>
          ...
          <p className="text-xs text-muted-foreground">Conhece p, E_ext, θ e o ponto de prova</p>
        </div>
      </div>
      <div ref={scrollRef} className="flex-1 overflow-y-auto p-4 space-y-3">
        {messages.length === 0 && (
          <div className="space-y-3 animate-fade-in">
            <p className="text-sm text-muted-foreground">Pergunte sobre torque, energia ou o campo do dipolo.</p>
      ...
      <form className="flex items-center gap-2 p-3 border-t border-border bg-background"
        ...
        <Input value={input} onChange={(e) => setInput(e.target.value)}
          placeholder="Pergunte algo sobre o dipolo elétrico..." disabled={loading} className="flex-1" />
