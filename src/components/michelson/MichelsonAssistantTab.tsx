...
const SUGGESTIONS = [
  "Por que aparecem anéis e não franjas retas?",
  "Como o Michelson mede comprimentos com precisão sub-micrométrica?",
  "O que muda no padrão se eu inserir vidro num dos braços?",
  "Qual é a relação com o experimento de Michelson–Morley?",
];

export const MichelsonAssistantTab = ({ params, results }: Props) => {
  ...
  const send = async (text: string) => {
    ...
    try {
      ...
      const summary = {
        modo: params.mode,
        lambda_nm: params.wavelengthNm,
        L1_mm: params.L1mm, L2_mm: params.L2mm,
        pathDiff_m: results.pathDiffM,
        ordemCentral: results.orderCenter,
        intensidadeCentral: results.centralIntensity,
        tilt_mrad: params.tiltMrad,
        espacamentoFranja_mm: results.fringeSpacingMm,
        aneisVisiveis: results.numVisibleRings,
        visibilidade: params.visibility,
      };
      const resp = await fetch(url, {
        ...
        body: JSON.stringify({
          ...
          context: { experimento: "Interferômetro de Michelson", parametros: params, resultados: summary },
        }),
      });
    ...
    <div className="flex flex-col h-[600px] rounded-xl border border-border bg-card shadow-card overflow-hidden">
      <div className="flex items-center gap-2 px-5 py-3 border-b border-border bg-gradient-subtle">
        ...
        <div>
          ...
          <p className="text-xs text-muted-foreground">Conhece λ, Δ, modo, visibilidade e contagem de franjas</p>
        </div>
      </div>
      <div ref={scrollRef} className="flex-1 overflow-y-auto p-4 space-y-3">
        {messages.length === 0 && (
          <div className="space-y-3 animate-fade-in">
            <p className="text-sm text-muted-foreground">Pergunte sobre franjas circulares/retilíneas, metrologia, FTIR ou Michelson–Morley.</p>
      ...
      <form className="flex items-center gap-2 p-3 border-t border-border bg-background"
        ...
        <Input value={input} onChange={(e) => setInput(e.target.value)}
          placeholder="Pergunte algo sobre o Michelson..." disabled={loading} className="flex-1" />
