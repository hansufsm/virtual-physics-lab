...
const SUGGESTIONS = [
  "Por que a f.e.m. é zero quando a espira está totalmente dentro da região de campo?",
  "Como a lei de Lenz determina o sentido da corrente induzida?",
  "Por que ε(t) tem dois lóbulos opostos quando o ímã atravessa a bobina?",
  "Como aumentar a f.e.m. máxima sem mudar o ímã?",
];

export const FaradayAssistantTab = ({ params, state }: Props) => {
    ...
    try {
      ...
      const resp = await fetch(url, {
        ...
        body: JSON.stringify({
          ...
          context: {
            experimento: "Indução eletromagnética (Faraday)",
            parametros: params,
            estado_atual: {
              t_s: state.t, pos_m: state.pos, v_ms: state.vel,
              flux_Wb: state.flux, emf_V: state.emf,
              corrente_A: state.current, potencia_W: state.power,
            },
          },
        }),
      });
    ...
    <div className="flex flex-col h-[600px] rounded-xl border border-border bg-card shadow-card overflow-hidden">
      <div className="flex items-center gap-2 px-5 py-3 border-b border-border bg-gradient-subtle">
        ...
        <div>
          ...
          <p className="text-xs text-muted-foreground">Conhece o cenário de indução atual</p>
        </div>
      </div>
        ...
        {messages.length === 0 && (
          <div className="space-y-3 animate-fade-in">
            <p className="text-sm text-muted-foreground">
              Pergunte sobre lei de Faraday, lei de Lenz, ou peça interpretação dos gráficos.
            </p>
      ...
      >
        <Input
          ...
          placeholder="Pergunte algo sobre Faraday/Lenz..."
          disabled={loading}
          className="flex-1"
        />
