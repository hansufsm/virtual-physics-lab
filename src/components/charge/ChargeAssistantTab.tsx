...
const SUGGESTIONS = [
  "Por que o campo magnético não realiza trabalho sobre a carga?",
  "Como o raio de Larmor depende da velocidade e da massa?",
  "Como ajusto E e B para o seletor passar v₀ = 5×10⁵ m/s?",
  "Por que no ciclotron a frequência não depende da velocidade?",
];

export const ChargeAssistantTab = ({ params, derived }: Props) => {
    ...
    try {
      ...
      const resp = await fetch(url, {
        ...
        body: JSON.stringify({
          ...
          context: {
            experimento: "Movimento de carga em campos E e B",
            parametros: params,
            derivados: {
              q_C: derived.q, m_kg: derived.m, qOverM: derived.qOverM,
              raioLarmor_m: derived.rGyro, fCiclotronica_Hz: derived.fCyc,
              vSeletor_ms: derived.vSelector, energia_eV: derived.energyEv,
            },
          },
        }),
      });
    ...
    <div className="flex flex-col h-[600px] rounded-xl border border-border bg-card shadow-card overflow-hidden">
      <div className="flex items-center gap-2 px-5 py-3 border-b border-border bg-gradient-subtle">
        ...
        <div>
          ...
          <p className="text-xs text-muted-foreground">Conhece os parâmetros da partícula e dos campos</p>
        </div>
      </div>
      ...
      <div ref={scrollRef} className="flex-1 overflow-y-auto p-4 space-y-3">
        {messages.length === 0 && (
          <div className="space-y-3 animate-fade-in">
            <p className="text-sm text-muted-foreground">Pergunte sobre força de Lorentz, raio de Larmor ou seletor de velocidades.</p>
      ...
      <form className="flex items-center gap-2 p-3 border-t border-border bg-background"
        ...
        <Input value={input} onChange={(e) => setInput(e.target.value)}
          placeholder="Pergunte algo sobre a trajetória..." disabled={loading} className="flex-1" />
