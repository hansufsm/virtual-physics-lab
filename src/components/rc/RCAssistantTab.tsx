...
const SUGGESTIONS = [
  "Por que Vc atinge 63% de E exatamente em t = τ?",
  "Como extrair τ a partir do gráfico linearizado?",
  "Se eu dobrar R, o que acontece com a corrente inicial?",
  "Qual a energia dissipada no resistor durante toda a descarga?",
];

export const RCAssistantTab = ({ params, results, t, vc, ic }: Props) => {
    ...
    try {
      ...
      const resp = await fetch(url, {
        ...
        body: JSON.stringify({
          ...
          context: {
            experimento: "Circuito RC — carga e descarga",
            parametros: params,
            resultados: {
              tau_s: results.tau,
              R_ohm: results.rOhm,
              C_F: results.cFarad,
              V_final_V: results.vFinal,
              i0_A: results.i0,
              t_atual_s: t,
              Vc_atual_V: vc,
              i_atual_A: ic,
    ...
    <div className="flex flex-col h-[600px] rounded-xl border border-border bg-card shadow-card overflow-hidden">
      <div className="flex items-center gap-2 px-5 py-3 border-b border-border bg-gradient-subtle">
        ...
        <div>
          ...
          <p className="text-xs text-muted-foreground">Conhece o circuito RC atual</p>
        </div>
      </div>
        ...
        {messages.length === 0 && (
          <div className="space-y-3 animate-fade-in">
            <p className="text-sm text-muted-foreground">
              Pergunte sobre carga/descarga, constante de tempo τ, ou peça interpretação dos gráficos.
            </p>
      ...
      >
        <Input
          ...
          placeholder="Pergunte algo sobre o RC..."
          disabled={loading}
          className="flex-1"
        />
