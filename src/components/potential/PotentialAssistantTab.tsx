...
import type { PotentialParams, PotentialDerived } from "@/lib/physics";

interface Msg { role: "user" | "assistant"; content: string }
interface Props { params: PotentialParams; derived: PotentialDerived }

const SUGGESTIONS = [
  "Por que E é sempre perpendicular às equipotenciais?",
  "Como deduzir V de um dipolo a grandes distâncias?",
  "Onde E = 0 em duas cargas iguais? E o que ocorre com V lá?",
  "Por que V de uma carga pontual decai com 1/r e |E| com 1/r²?",
];

export const PotentialAssistantTab = ({ params, derived }: Props) => {
    ...
    try {
      ...
      const resp = await fetch(url, {
        ...
        body: JSON.stringify({
          ...
          context: { experimento: "Potencial elétrico e equipotenciais", parametros: params, derivados: derived },
        }),
      });
    ...
    <div className="flex flex-col h-[600px] rounded-xl border border-border bg-card shadow-card overflow-hidden">
      <div className="flex items-center gap-2 px-5 py-3 border-b border-border bg-gradient-subtle">
        ...
        <div>
          ...
          <p className="text-xs text-muted-foreground">Conhece a distribuição de cargas e o ponto de prova</p>
        </div>
      </div>
      <div ref={scrollRef} className="flex-1 overflow-y-auto p-4 space-y-3">
        {messages.length === 0 && (
          <div className="space-y-3 animate-fade-in">
            <p className="text-sm text-muted-foreground">Pergunte sobre V, equipotenciais ou a relação E = −∇V.</p>
      ...
      <form className="flex items-center gap-2 p-3 border-t border-border bg-background"
        ...
        <Input value={input} onChange={(e) => setInput(e.target.value)}
          placeholder="Pergunte algo sobre o potencial elétrico..." disabled={loading} className="flex-1" />
