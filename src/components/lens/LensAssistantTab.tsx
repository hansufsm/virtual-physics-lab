...
const SUGGESTIONS = [
  "Por que d₀ = f produz imagem no infinito?",
  "Como deduzo m = −dᵢ/d₀ a partir de triângulos semelhantes?",
  "Quando uma lente convergente produz imagem virtual?",
  "Por que a potência de uma lente cai quando ela é imersa em água?",
];

export const LensAssistantTab = ({ params, results }: Props) => {
  ...
  const send = async (text: string) => {
    ...
    try {
      ...
      const summary = {
        f_cm: results.focalCm,
        potencia_D: results.power,
        di_cm: results.imageDistanceCm,
        hi_cm: results.imageHeightCm,
        m: results.magnification,
        tipo_imagem: results.imageType,
        orientacao: results.orientation,
        tamanho: results.size,
        convergente: results.isConverging,
      };
      const resp = await fetch(url, {
        ...
        body: JSON.stringify({
          ...
          context: { experimento: "Lentes finas e óptica geométrica", parametros: params, resultados: summary },
        }),
      });
    ...
    <div className="flex flex-col h-[600px] rounded-xl border border-border bg-card shadow-card overflow-hidden">
      <div className="flex items-center gap-2 px-5 py-3 border-b border-border bg-gradient-subtle">
        ...
        <div>
          ...
          <p className="text-xs text-muted-foreground">Conhece f, d₀, h₀ e o tipo de lente configurado</p>
        </div>
      </div>
      <div ref={scrollRef} className="flex-1 overflow-y-auto p-4 space-y-3">
        {messages.length === 0 && (
          <div className="space-y-3 animate-fade-in">
            <p className="text-sm text-muted-foreground">Pergunte sobre Gauss, Newton, equação dos fabricantes ou formação de imagem.</p>
      ...
      <form className="flex items-center gap-2 p-3 border-t border-border bg-background"
        ...
        <Input value={input} onChange={(e) => setInput(e.target.value)}
          placeholder="Pergunte algo sobre lentes..." disabled={loading} className="flex-1" />
